...
export default function AuditLogsPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Header */}
      <div>
        <h1 className="text-2xl font-extrabold text-dark dark:text-white">Audit Logs</h1>
      ...
      <div className="bg-white border border-stroke dark:border-dark-3 rounded-[10px] shadow-1 overflow-hidden">
        {loading ? (
          <div className="py-20 flex flex-col justify-center items-center">
            ...
            <p className="text-xs font-semibold text-dark-5 dark:text-dark-6">Loading system logs...</p>
          </div>
        ) : logs.length === 0 ? (
          <div className="py-20 text-center">
            <p className="text-sm font-semibold text-dark-5 dark:text-dark-6">No administrative logs recorded yet.</p>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-stroke dark:divide-dark-3 text-dark dark:text-white text-xs font-semibold">
                {logs.map((log) => {
                    ...
                    <>
                      <tr key={log.id} className="hover:bg-gray-50/30 transition-colors">
                        <td className="py-4 px-6">
                          ...
                          <span className="block text-[9px] text-dark-6 font-normal font-mono">{log.adminAuthUserId}</span>
                        </td>
                        <td className="py-4 px-6">
                        ...
                        <td className="py-4 px-6">
                          ...
                          <span className="block text-[9px] text-dark dark:text-white/45 font-normal font-mono">ID: {log.targetId}</span>
                        </td>
                        <td className="py-4 px-6 text-dark-5 dark:text-dark-6 font-medium">
                          {new Date(log.createdAt).toLocaleString()}
                        </td>
                        <td className="py-4 px-6 text-right">
                          <button
                            onClick={() => toggleExpandMetadata(log.id)}
                            className="inline-flex items-center px-2.5 py-1.5 rounded-lg border border-stroke dark:border-dark-3 text-[10px] font-bold text-dark dark:text-white hover:bg-gray-50 transition-colors"
                          >
                            {isExpanded ? (
                              <>
                                <EyeOff className="h-3.5 w-3.5 mr-1 text-primary" /> Hide payload
                              </>
                            ) : (
                              <>
                                <Eye className="h-3.5 w-3.5 mr-1 text-primary" /> Inspect data
                              </>
                            )}
                          </button>
                        </td>
                      ...
                      {isExpanded && (
                        <tr className="bg-gray-50/70 border-b border-stroke dark:border-dark-3">
                          <td colSpan={5} className="p-4 px-6">
                            <div className="bg-white border border-stroke dark:border-dark-3 rounded-xl p-4 shadow-inner space-y-2">
