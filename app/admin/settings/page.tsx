...
export default function SettingsPage() {
  ...
  if (loading) {
    return (
      <div className="py-20 flex flex-col justify-center items-center bg-white border border-stroke dark:border-dark-3 rounded-[10px] shadow-1">
        ...
        <p className="text-xs font-semibold text-dark-5 dark:text-dark-6">Loading system parameters...</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 max-w-2xl mx-auto">
      ...
      {/* Header */}
      <div>
        <h1 className="text-2xl font-extrabold text-dark dark:text-white">System Settings</h1>
      ...
      <div className="bg-white border border-stroke dark:border-dark-3 rounded-[10px] p-6 shadow-1">
        <h2 className="text-sm font-extrabold text-dark dark:text-white mb-6 uppercase tracking-wider flex items-center border-b border-stroke dark:border-dark-3 pb-2">
          <Sliders className="h-4.5 w-4.5 mr-2 text-primary" /> Platform Variables
        </h2>

        <form onSubmit={handleSave} className="space-y-6">
          
          {success && (
            <div className="p-4 bg-emerald-50 border border-emerald-200 rounded-xl text-emerald-800 text-xs font-bold flex items-center shadow-1">
              <CheckCircle2 className="h-5 w-5 mr-2" /> System parameters updated successfully!
            </div>
          )}

          {/* Application open/close status */}
          <div className="flex items-center justify-between p-4 border border-stroke dark:border-dark-3 rounded-xl hover:bg-gray-50/20 transition-colors">
            <div className="space-y-1 pr-4">
              <h4 className="text-xs font-bold text-dark dark:text-white">Application Sign-ups Open</h4>
              <p className="text-[10px] text-dark-5 dark:text-dark-6 font-semibold leading-relaxed">
            ...
            <button
              ...
              className="text-primary focus:outline-none transition-transform active:scale-95"
            >
              {applicationOpen ? (
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full border border-stroke dark:border-dark-3 rounded-xl pl-10 pr-3 py-2.5 text-xs font-semibold focus:outline-none focus:ring-2 focus:ring-primary text-dark dark:text-white"
              />
            </div>
            <p className="text-[9px] text-dark-6 font-medium">
